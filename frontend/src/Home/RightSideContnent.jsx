...
function RightSideContnent() {
  ...
  const selectedDateRange = useSelector((state) => state.dateRange.dateRange);

  console.log(selectedDateRange);
  const getIncomeOfTheMonth = async (selectedDateRange) => {
    let income = await getMonthlyIncome(userId, selectedDateRange);
    setMonthlyIncome(income[0]?.totalIncome ?? 0);
  };

  const getExpenseOfTheMonth = async (selectedDateRange) => {
    let expense = await getMonthlyExpenses(userId, selectedDateRange);
    setMonthlyExpenses(expense[0]?.totalExpenses ?? 0);
  };

  useEffect(() => {
    getIncomeOfTheMonth(selectedDateRange);
    getExpenseOfTheMonth(selectedDateRange);
  }, [selectedDateRange]);
  return (
    <div>
      {/* settings */}
      ...
      <div className=" h-24 md:w-3/4 lg:w-2/4 shadow-lg  ml-10 lg:ml-10 rounded-md lg:mt-20  mr-10 border mt-10">
        <div className="flex justify-between">
          ...
          <p className="ml-5 mt-6 mr-5 text-[10px]">{selectedDateRange}</p>
        </div>
      ...
      <div className=" h-24 md:w-3/4 lg:w-2/4 shadow-lg  ml-10 lg:ml-10 rounded-md  mt-10 lg:mt-6  mr-10 border ">
        ...
        <p className="ml-5 font-bold font-poppins text-lg">
          £ {monthlyExpesnse}
        </p>
      </div>
